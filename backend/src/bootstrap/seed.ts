...
const COURSES: SeedCourse[] = [
    ...
    lessons: [
      {
        ...
        summary: 'Why most of your tree should never ship JavaScript to the browser.',
        durationMinutes: 12,
        content:
      ...
      },
      {
        ...
        summary: 'async components, request deduplication, and the cache you get for free.',
        durationMinutes: 15,
        content:
      ...
      },
      {
        ...
        summary: 'Replacing the API-route-plus-fetch dance with a function call.',
        durationMinutes: 14,
        content:
    ...
    quizzes: [
      {
        ...
        description: 'Six questions on server components, data fetching and where to enforce rules.',
        passingScore: 70,
        questions: [
          {
            text: 'In the Next.js App Router, what is a component by default?',
            options: ['A client component', 'A server component', 'A static HTML file', 'An API route'],
            correctOptionIndex: 1,
            explanation: 'Everything is a server component until a file is marked "use client".',
          },
          {
            text: 'Which caching option should you use for a response that differs per signed-in user?',
            options: ['cache: "force-cache"', 'cache: "no-store"', 'next: { revalidate: 3600 }', 'No option needed'],
            correctOptionIndex: 1,
            explanation: 'Caching a personalised response can serve one user\'s data to another.',
          },
          {
          ...
          },
          {
            ...
            explanation: 'A request straight to the backend never passes through frontend middleware.',
          },
          {
          ...
          },
          {
            ...
            correctOptionIndex: 1,
            explanation: 'It is reachable by anyone, so it needs the same checks as a REST handler.',
          },
        ],
      },
    ],
  },
  {
    ...
    summary: 'Types that describe what your code actually does — inference, narrowing and generics.',
    description:
    ...
    lessons: [
      {
        ...
        summary: 'Most annotations you write are noise the compiler already knew.',
        durationMinutes: 9,
        content:
      ...
      },
      {
        ...
        summary: 'Making illegal states unrepresentable, then proving to the compiler you handled them.',
        durationMinutes: 13,
        content:
        ...
        questions: [
          {
            ...
            correctOptionIndex: 2,
            explanation: 'Which is why an explicit annotation matters for empty arrays.',
          },
          {
          ...
          },
          {
            ...
            options: ['A `typeof` check', 'A comment', 'A type assertion with `as`', 'Renaming the variable'],
            correctOptionIndex: 0,
            explanation: '`as` overrides the compiler rather than proving anything to it.',
          },
        ],
      },
    ],
  },
  {
    ...
    summary: 'Content types, custom controllers, and authorization that holds up to a direct curl request.',
    description:
        ...
        questions: [
          {
            ...
            correctOptionIndex: 1,
            explanation: 'Role-only rules need no data, so they can run before anything is loaded.',
          },
          {
  ...
  },
  {
    ...
    summary: 'Reading query plans, choosing indexes, and finding the query that is actually slow.',
    description:
    ...
    lessons: [
      ...
      },
      {
        ...
        summary: 'Column order, selectivity, and why your index is being ignored.',
        durationMinutes: 14,
        content:
...
const POSTS: SeedPost[] = [
  ...
  },
  {
    ...
    excerpt: 'An unpublished draft — visible to its author and to admins, invisible to everyone else.',
    tags: 'announcements',
    readingMinutes: 2,
    status: 'draft',
    coverImageUrl:
...
async function ensureUser(strapi: any, seed: SeedUser, roleIdByType: Record<string, number>) {
...
export async function seedDemoData(strapi: any) {
  ...
  strapi.log.info('[seed] empty database detected — creating demo accounts and content');
  ...
  for (const seed of COURSES) {
    ...
    const course = await strapi.documents(COURSE_UID).create({
      data: {
        ...
        durationMinutes: seed.lessons.reduce((sum, lesson) => sum + lesson.durationMinutes, 0),
...
async function enrol(strapi: any, student: any, course: any, completedLessons: number) {
  if (!course) return;
  ...
  const totalLessons = await strapi.db.query(LESSON_UID).count({ where: { course: course.id } });
...
async function recordQuizAttempt(strapi: any, student: any, course: any) {
  ...
  const answers = quiz.questions.map((question: any, index: number) => ({
    ...
    selectedOptionIndex:
      index === 0
        ? (Number(question.correctOptionIndex ?? 0) + 1) % (question.options?.length || 1)
        : Number(question.correctOptionIndex ?? 0),
  }));

  const correctCount = answers.filter(
    (answer: any, index: number) => answer.selectedOptionIndex === Number(quiz.questions[index].correctOptionIndex ?? 0)
  ).length;
