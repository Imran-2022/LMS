...
export async function findCourse(
  ...
  if (/^\d+$/.test(key)) {
    return strapi.db
      .query(COURSE_UID)
      .findOne({ where: { id: Number(key) }, populate });
  }
...
export async function findParentCourse(
  ...
  const where = /^\d+$/.test(key)
    ? { $or: [{ documentId: key }, { id: Number(key) }] }
    : { documentId: key };
...
export async function assertCourseReadAccess(
  ...
  if (isInstructor(user)) {
    ...
    throw new ForbiddenError(
      "You can only open lessons from your own courses.",
    );
  }
...
export async function assertProgressReadAccess(
  ...
  if (isInstructor(user)) {
    ...
    if (ownerId && Number(ownerId) === Number(user.id)) return;
    throw new ForbiddenError(
      "You can only review progress for your own courses.",
    );
  }
