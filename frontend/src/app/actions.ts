...
export async function enroll(courseId: number) {
  await request("/api/enrollments", "POST", { courseId });
  redirect("/my-courses");
}
export async function completeLesson(lessonId: number, courseId: number) {
  await request(`/api/lesson-progress/${lessonId}/complete`, "POST");
