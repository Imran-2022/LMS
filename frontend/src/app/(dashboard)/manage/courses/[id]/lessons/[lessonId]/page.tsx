...
export default async function ManageLessonPage({ params }: { params: Promise<{ id: string; lessonId: string }> }) {
  ...
  return <><PageHeader eyebrow={`Course ${id}`} title="Edit lesson" description="Update lesson content and keep the learning path in order." /><div className="mt-8 max-w-3xl rounded-2xl border border-ink-200 bg-white p-6 sm:p-8"><LessonForm courseId={id} lesson={lesson} /></div></>;
}