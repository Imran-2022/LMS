...
export default async function ManageQuizPage({ params }: { params: Promise<{ id: string; quizId: string }> }) {
  ...
  return <><PageHeader eyebrow={`Course ${id}`} title={quiz.title} description="Update questions, answers, and grading settings." /><div className="mt-8 max-w-4xl rounded border border-ink-200 bg-white p-6 sm:p-8"><QuizForm courseId={id} quiz={quiz} /></div></>;
}