...
export default async function AdminCoursesPage() {
  await requireAdmin();
  const courses = await fetchList<AdminCourse>("/api/admin/courses");
  return (
    <>
      <PageHeader
        eyebrow="Admin"
        title="All courses"
        description="Review and manage every course on the platform."
      />
          ...
          <tbody className="divide-y divide-ink-100">
            {courses.map((course) => (
              <tr key={course.id}>
                ...
                <td className="px-5 py-4">{course.status}</td>
